...
// PaymentService is the interface the billing usecase depends on.
// The real Client and any test double must implement this.
...
// caller is the function that actually makes the gRPC call.
// Extracted so tests can inject a fake without a real gRPC server.
...
func newWithCaller(fn caller) *Client {
	cb := gobreaker.NewCircuitBreaker[*CreatePaymentResult](gobreaker.Settings{
		...
		MaxRequests: 1,                // 1 probe request in HALF-OPEN
		Interval:    60 * time.Second, // reset failure counts every 60s in CLOSED
		Timeout:     30 * time.Second, // wait 30s in OPEN before trying HALF-OPEN
